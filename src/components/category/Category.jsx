...
const Category = () => {
  ...
  const options = [
    ...
    },
    {
      key: "price-asc",
      label: "Giá Thấp - Cao",
      icon: <BiSortUp className="text-black" />,
    },
    {
      key: "price-desc",
      label: "Giá Cao - Thấp",
      icon: <BiSortDown className="text-black" />,
    },
  ];
  const handleSort = (sortKey) => {
    ...
    if (sortKey === "price-asc") {
      const sorted = [...products].sort((a, b) => {
        const priceA =
          a.productVariants?.[0]?.priceSale ||
          a.productVariants?.[0]?.price ||
          0;
        const priceB =
          b.productVariants?.[0]?.priceSale ||
          b.productVariants?.[0]?.price ||
          0;
        ...
        {loading ? (
          ...
          <div className="text-center w-full py-10 text-gray-600">
            <p className="text-lg font-semibold">
              Không có sản phẩm nào từ danh mục này.
            </p>
          </div>
        ) : (
